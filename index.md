...
Never before has it been so easy to learn so deeply. Marvin was born to be hacked, with few dependencies and using only basic C++. All code lives in [```marvin.hpp```](https://github.com/PrincetonVision/marvin/blob/master/marvin.hpp) and [```marvin.cu```](https://github.com/PrincetonVision/marvin/blob/master/marvin.cu). Win friends and influence people today in four easy steps:
